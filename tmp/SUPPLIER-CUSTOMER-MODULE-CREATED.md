...
**Campos:**
- ✅ `supplier_customer_id` - ID único (PK, auto increment)
- ✅ `company_id` - FK para Company (obrigatório)
- ✅ `supplier_customer_name` - Nome (obrigatório)
- ✅ `supplier_customer_document` - CPF/CNPJ (opcional)
- ✅ `supplier_customer_email` - E-mail (opcional)
- ✅ `supplier_customer_phone` - Telefone (opcional)
- ✅ `supplier_customer_address` - Endereço (opcional)
- ✅ `supplier_customer_is_active` - Status ativo (obrigatório, padrão true)
- ✅ Campos de auditoria: `criado_por`, `atualizado_por`, `criado_em`, `atualizado_em`

**Índices:**
- ✅ `uk_supplier_customer_company_name` - UNIQUE(company_id, supplier_customer_name)

**Foreign Keys:**
- ✅ `fk_supplier_customer_company` → `tb_company(company_id)`

---

## 🔗 Relações Criadas

### **1. FinancialTransaction → SupplierCustomer**

**Modificações em `tb_financial_transaction`:**
```sql
...
### **2. AccountPayableReceivable → SupplierCustomer**

**Modificações em `tb_account_payable_receivable`:**
```sql
ALTER TABLE ADD COLUMN "supplier_customer_id" bigint NULL;
...
A tabela **NÃO possui campo `type`** porque:
- ✅ Um mesmo fornecedor pode ser cliente e vice-versa
- ✅ A classificação de despesa/receita vem da **transação** (FinancialTransaction ou AccountPayableReceivable)
- ✅ Evita duplicação de cadastros
- ✅ Mais flexível e realista

**Exemplo:**
- Empresa X compra produtos de Fornecedor Y (despesa)
- Empresa X vende serviços para Fornecedor Y (receita)
...
### **1. Entidade**
✅ `backend/-1-Domain/Entities/supplierCustomer.cs`
- Propriedades mapeadas
- Relações virtuais com Company, AccountPayableReceivable, FinancialTransaction
- Construtor padrão e com parâmetros

### **2. DTOs**
✅ `backend/-2-Application/DTOs/SupplierCustomer/SupplierCustomerFilterDTO.cs`
✅ `backend/-2-Application/DTOs/SupplierCustomer/SupplierCustomerInputDTO.cs`
✅ `backend/-2-Application/DTOs/SupplierCustomer/SupplierCustomerOutputDTO.cs`

**SupplierCustomerInputDTO:**
- Nome (obrigatório)
- Document, Email, Phone, Address (opcionais)
- IsActive (obrigatório)
- **CompanyId NÃO está presente** (preenchido pelo Service)
...
- ✅ Adicionado `public long? SupplierCustomerId { get; set; }`
- ✅ Adicionado `public virtual SupplierCustomer SupplierCustomer { get; set; } = null!;`
- ✅ Construtor atualizado com parâmetro `SupplierCustomerId`
...
**`erp.sql`:**
- ✅ Tabela `tb_supplier_customer` criada
- ✅ Coluna `supplier_customer_id` adicionada em `tb_financial_transaction`
- ✅ Coluna `supplier_customer_id` adicionada em `tb_account_payable_receivable`
...
### **Passo 3: Verificar**
O script já exibe verificações automáticas. Você verá:
- ✅ Confirmação da criação da tabela `tb_supplier_customer`
- ✅ Lista das colunas `supplier_customer_id` adicionadas
...
⚠️ **ATENÇÃO:** Só use em desenvolvimento! Isso remove dados.
```bash
...
Para completar o módulo, você precisará criar:
...
10. **Migração de Banco** ✅
    - ✅ `004_add_supplier_customer.sql` criado em `database/migrations/`
    - ✅ `004_rollback_supplier_customer.sql` criado (para reverter se necessário)
...
- **Índices criados:** 1 (unique)
- **Scripts de migração:** 2 (apply + rollback)
